...
class RLCT:
    ...
    def fit(self ,dataset_name, states_train, train_y, states_cal, cal_y, states_test, test_y, MAX_EPISODES, action_low, action_high):
        ...
        for episode  in range(MAX_EPISODES):
            ...
            Test_actions = agent.regression_prediction(states_val)
            Test_actions = torch.reshape(torch.tensor(Test_actions), (val_y.shape[0],1))
            final_result = agent.critic_criterion(Test_actions, torch.tensor(val_y))
            print('Episode %d final result: %.2f'%(episode+1,final_result))
            
            if final_result <= initial_mse:
                model_states = {
                                'actor': agent.actor.state_dict()  }
                torch.save(model_states, os.getcwd() + f'\\{dataset_name}\\SAC_best_agent_{s}.pth')
                initial_mse = final_result
        
        
            model_states = torch.load(f'{dataset_name}\\SAC_best_agent_{s}.pth')        
