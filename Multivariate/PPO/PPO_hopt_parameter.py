...
def objective(params):
    ...
    for step in range(index, states_train.shape[0]-1):
        ...
        reward = -(abs(action[0][0]-train_y[step][0]) + abs(action[0][1]-train_y[step][1]))/2 
    ...
    log_prob_old = torch.reshape(log_prob_old, (log_prob_old.shape[0],1))
    returns = (model.calculate_returns(Rewards, model.gamma)).to(device)
    returns = (torch.reshape(returns, (returns.shape[0],1))).float()
    ...
    Val_actions1, Val_actions2 = model.regression_prediction(states_val)
    ...
    Reward = (model.MseLoss(Val_actions1, torch.tensor(val_y[:,0].reshape(val_y.shape[0],1))) + model.MseLoss(Val_actions2, torch.tensor(val_y[:,1].reshape(val_y.shape[0],1))))/2
    print(Reward)
    return Reward.item()
...
for dataset_name in datasets:
    ...
    states_train, states_val, train_y, val_y = one_dimen_transform(train_data, val_data, look_back, 2)
    _, states_test, _, test_y = one_dimen_transform(val_data, test_data, look_back, 2)
