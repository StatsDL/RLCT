...
class QRNN(torch.nn.Module):
    def __init__(
        ...
        self.mlp = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_size, 2 * horizon * output_size)
        )
    ...
    def forward(self, x):
        ...
        out = out.reshape(-1, self.horizon, self.OUTPUT_SIZE, 2)
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.EPOCH):
            ...
            y = y.reshape(-1, self.horizon, self.OUTPUT_SIZE, 2)
            output = self(x).reshape(-1, self.horizon, self.OUTPUT_SIZE, 2)
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            X_test = X.type(torch.FloatTensor)
        
        predicts_ = self(X_test).view(-1, self.horizon, self.OUTPUT_SIZE, 2)
        prediction_0 = predicts_[:, :, :, 0].squeeze()
        prediction_1 = predicts_[:, :, :, 1].squeeze()
...
for dataset_name in datasets:
    ...
    for s in range(seeds):
        model = QRNN(epochs=100, batch_size=32, input_size = 20, learning_rate = 0.001, output_size = 1, horizon = 2, hidden_size = 64, num_layers = 1, alpha = 0.1)
