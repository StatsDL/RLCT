...
class AuxiliaryForecaster(torch.nn.Module):
    ...
    def __init__(self, embedding_size, input_size=1, output_size=1, rnn_mode="LSTM", path=None):
        """
        ...
        **kwargs
        ...
        if self.auxiliary_forecaster_path and os.path.isfile(self.auxiliary_forecaster_path):
            ...
            self.auxiliary_forecaster = AuxiliaryForecaster(
                embedding_size, input_size, output_size,  rnn_mode, auxiliary_forecaster_path
            )
...
for dataset_name in datasets:
    ...
    for s in range(seeds):
        ...
        model = CFRNN(embedding_size = 32, input_size = 20, output_size = 1, error_rate = 0.1)
        model.fit(train_dataset, calibration_dataset, epochs = 100, lr = 0.001, batch_size =32)
        
        independent_coverages, joint_coverages, intervals = model.evaluate_coverage(test_dataset, corrected = False)
        mean_independent_coverage = torch.mean(independent_coverages.float(), dim=0)*100
        interval_width = np.mean((test_scaler.inverse_transform(intervals[:,1].detach().numpy()) - test_scaler.inverse_transform(intervals[:,0].detach().numpy())).squeeze(), axis=0)
        
        # Plot the bounds themselves
        plt.plot(np.arange(len(y_test)), test_scaler.inverse_transform(intervals[:, 0].detach().numpy()), color='blue', label='Lower Bound')
        plt.plot(np.arange(len(y_test)), test_scaler.inverse_transform(intervals[:, 1].detach().numpy()), color='red', label='Upper Bound')
        
        # Plot y
        plt.plot(np.arange(len(y_test)), test_scaler.inverse_transform(y_test.detach().numpy()), color='black', label='y')
